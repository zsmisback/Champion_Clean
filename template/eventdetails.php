  ...
  <table class="table table-responsive mb-4">
    ...
    <tbody>
     ...
     <h3 class="mb-2"><?php if($response['seats'] == 'seats'){echo '<span>Seats</span>';} if($response['locker_room'] == 'locker_room'){echo ' <span>Locker Room</span>';} if($response['showers'] == 'showers'){echo ' <span>Showers</span>';}?> </h3><br>
				   ...
				   elseif($_SESSION['uid'] == $response['uid'])
				   {
					   echo "";
				   }
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post">
		<input type = "hidden" name="event_enquiries|enquiry_for" value="<?php echo $response['uid']; ?>"/>
		<input type = "hidden" name="event_enquiries|enquiry_by" value="<?php echo $_SESSION['uid']; ?>"/>
		<input type = "hidden" name="event_enquiries|name" value="<?php echo $_SESSION["name"]; ?>"/>
		<input type = "hidden" name="event_enquiries|email" value="<?php echo $_SESSION["username"]; ?>"/>
		<input type = "hidden" name="event_enquiries|contact_number" value="<?php echo $_SESSION["contact_no"]; ?>"/>
		<input type = "hidden" name="event_enquiries|event_name" value="<?php echo $response['name']; ?>"/>
		Select the sport that you're interested in:<br><br>
		<?php
		
		foreach($sports as $sport)
		{
			echo'<input type="checkbox" id="crick" name="event_enquiries|sports[]" value="'.$sport.'">
				<label for="crick">'.ucfirst($sport).'</label><br>
				';
		}
		...
		?>
          ...
          <div class="form-group">
            ...
            <input type="number" class="form-control" id="recipient-name" name="event_enquiries|alt_contact_number">
          </div>
          <div class="form-group">
            <label for="message-text" class="col-form-label">Message:</label>
            <textarea class="form-control" id="message-text" name="event_enquiries|query"></textarea>
          </div>
